...
std::string
Path::CombinePaths (const std::string &path1, const std::string &path2)
{
  if (path1 == "")
    return path2;
  if (path2 == "")
...
std::string
Path::GetDirectoryPath (const std::string &path)
{
...
Path::Split (const std::string &s, char delim)
{
...
bool
Path::GetHostPortAndPath (const std::string &url, std::string &host, size_t &port,
                          std::string &path)
{
  std::string hostPort = "";
  size_t found = 0;
...
bool
Path::GetStartAndEndBytes (const std::string &byteRange, size_t &startByte, size_t &endByte)
{
  size_t found = 0;
