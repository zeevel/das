...
BlockStream::BlockStream () : length (0)
{
}
...
void
BlockStream::PushBack (block_t *block)
{
  this->length += block->len;
  this->blockqueue.push_back (block);
}
void
BlockStream::PushFront (block_t *block)
{
  this->length += block->len;
  this->blockqueue.push_front (block);
}
const block_t *
BlockStream::GetBytes (uint32_t len)
{
...
size_t
BlockStream::GetBytes (uint8_t *data, size_t len)
{
  /* Performance Intensive */
  if (len > this->length)
...
size_t
BlockStream::PeekBytes (uint8_t *data, size_t len)
{
  /* Performance Intensive */
  if (len > this->length)
    len = (size_t) this->length;
...
size_t
BlockStream::PeekBytes (uint8_t *data, size_t len, size_t offset)
{
  /* Performance Intensive */
  if (len > this->length)
    len = (size_t) this->length;
...
uint64_t
BlockStream::Length () const
{
  return this->length;
}
const block_t *
BlockStream::GetFront ()
{
  if (this->blockqueue.empty ())
    return NULL;
...
bool
BlockStream::BlockQueueGetBytes (uint8_t *data, uint32_t len)
{
  uint32_t pos = 0;
...
bool
BlockStream::BlockQueuePeekBytes (uint8_t *data, uint32_t len, size_t offset)
{
  uint32_t pos = 0;
  int cnt = 0;
...
uint8_t
BlockStream::ByteAt (uint64_t position) const
{
...
void
BlockStream::EraseFront (uint64_t len)
{
  if (len > this->length)
    len = this->length;
...
BlockStream *
BlockStream::GetBlocks (uint64_t len)
{
  if (len > this->length)
    return NULL;
