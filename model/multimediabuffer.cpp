...
using namespace ns3::dash::player;
...
MultimediaBuffer::~MultimediaBuffer() { buff.clear(); }

bool MultimediaBuffer::addToBuffer(
  ...
  if (toBufferSegmentNumber < segmentNumber) return false;

  // fprintf(stderr, "toBufferSegmentNumber=%d <
  ...
  if (usedRepresentation->GetDependencyId().size() > 0) {
    ...
    for (std::vector<std::string>::const_iterator k =
             usedRepresentation->GetDependencyId().begin();
    ...
    if (isFull(usedRepresentation->GetId(), duration)) return false;
  }
...
bool MultimediaBuffer::enoughSpaceInLayeredBuffer(
    unsigned int segmentNumber,
    const dash::mpd::IRepresentation *usedRepresentation) {
  // determine segment duration
  double duration = (double)usedRepresentation->GetSegmentList()->GetDuration();
...
bool MultimediaBuffer::enoughSpaceInTotalBuffer(
    unsigned int segmentNumber,
    const dash::mpd::IRepresentation *usedRepresentation) {
  // determine segment duration
  double duration = (double)usedRepresentation->GetSegmentList()->GetDuration();
  ...
  if (isFull(duration)) return false;
...
bool MultimediaBuffer::isEmpty() {
  if (getBufferedSeconds() <= 0.0) return true;
  return false;
}
...
/** get buffered seconds only from segments belonging to the representation id
 * repId */
...
MultimediaBuffer::consumeFromBuffer() {
  BufferRepresentationEntry entryConsumed;

  if (isEmpty()) return entryConsumed;
...
MultimediaBuffer::BufferRepresentationEntry
MultimediaBuffer::getHighestConsumableRepresentation(int segmentNumber) {
  ...
  for (BufferRepresentationEntryMap::iterator k = map.begin(); k != map.end();
       ++k) {
...
void MultimediaBuffer::traverseBuff() {
  std::cout << std::endl;
  std::cout << "********* 遍历播放缓存-start *************" << std::endl;
  MBuffer::iterator buffIter;
  for (buffIter = buff.begin(); buffIter != buff.end(); buffIter++) {
    std::cout << "buff key(SegmentNumber): " << buffIter->first << std::endl;
    std::cout << "buff value(BufferRepresentationEntryMap): " << std::endl;
    BufferRepresentationEntryMap buffItem = buffIter->second;
    BufferRepresentationEntryMap::iterator iter;
    for (iter = buffItem.begin(); iter != buffItem.end(); iter++) {
      std::cout << "\tbuff item's value's key(Representation, string): "
                << iter->first << std::endl;
      // BufferRepresentationEntry entry = iter->second;ss

      std::cout << "\tbuff item's value's value(BufferRepresentationEntry): "
                << std::endl;
      // printf("\t\tsegmentNumber=%d, segmentDuration=%ld, repId=%s, depIds={",
      //        iter->second.segmentNumber, iter->second.segmentDuration,
      //        iter->second.repId);
      std::cout << "\t\t segmentNumber, segmentDuration, repId is: "
                << iter->second.segmentNumber << ", "
                << iter->second.segmentDuration << ", " << iter->second.repId
                << ", depIds: {";
      for (auto depId : iter->second.depIds) {
        std::cout << depId << ", ";
      }
      std::cout << "}" << std::endl;
    }
