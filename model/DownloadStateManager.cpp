...
DownloadStateManager::DownloadStateManager () : state (NOT_STARTED)
{
...
DownloadState
DownloadStateManager::State () const
{
...
void
DownloadStateManager::State (DownloadState state)
{
  EnterCriticalSection (&this->stateLock);
...
void
DownloadStateManager::WaitState (DownloadState state) const
{
  EnterCriticalSection (&this->stateLock);
...
void
DownloadStateManager::CheckAndWait (DownloadState check, DownloadState wait) const
{
  EnterCriticalSection (&this->stateLock);
...
void
DownloadStateManager::Attach (IDownloadObserver *observer)
{
  EnterCriticalSection (&this->stateLock);
...
void
DownloadStateManager::Detach (IDownloadObserver *observer)
{
  EnterCriticalSection (&this->stateLock);
...
void
DownloadStateManager::Notify ()
{
  for (size_t i = 0; i < this->observers.size (); i++)
    this->observers.at (i)->OnDownloadStateChanged (this->state);
}
void
DownloadStateManager::CheckAndSet (DownloadState check, DownloadState set)
{
  EnterCriticalSection (&this->stateLock);
