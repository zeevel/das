...
MultimediaPlayer("dash::player::AdaptationLogic")
{
  m_lastBitrate = 0.0;
}*/
...
#endif
  if (m_adaptLogic) delete m_adaptLogic;
  m_adaptLogic = NULL;
...
MultimediaPlayer::MultimediaPlayer(std::string AdaptationLogicStr,
                                   unsigned int maxBufferedSeconds) {
  m_buffer = new MultimediaBuffer(maxBufferedSeconds);
  m_lastBitrate = 0;
  AdaptationLogic *aLogic =
      AdaptationLogicFactory::Create(AdaptationLogicStr, this);

  if (aLogic == NULL) {
    std::cerr << "MultimediaPlayer():\tFailed initializing adaptation logic '"
              << AdaptationLogicStr << "'" << std::endl;
  } else {
#if defined(DEBUG) || defined(NS3_LOG_ENABLE)
    std::cerr << "MultimediaPlayer():\tInitialized adaptation logic of type "
              << aLogic->GetName() << std::endl;
#endif
    m_adaptLogic = aLogic;
  }
}

AdaptationLogic *MultimediaPlayer::GetAdaptationLogic() { return m_adaptLogic; }

void MultimediaPlayer::SetAvailableRepresentations(
...
bool MultimediaPlayer::AddToBuffer(
  ...
  return m_buffer->addToBuffer(segmentNr, usedRepresentation,
                               experiencedDownloadBitrate);
}

bool MultimediaPlayer::EnoughSpaceInBuffer(
    unsigned int segmentNr,
    const dash::mpd::IRepresentation *usedRepresentation,
...
double MultimediaPlayer::GetBufferLevel(std::string repId) {
...
double MultimediaPlayer::GetBufferPercentage(std::string repId) {
  if (repId.compare("NULL") == 0)
...
unsigned int MultimediaPlayer::getHighestBufferedSegmentNr(std::string repId) {
  return m_buffer->getHighestBufferedSegmentNr(repId);
}

MultimediaBuffer::BufferRepresentationEntry
MultimediaPlayer::ConsumeFromBuffer() {
  return m_buffer->consumeFromBuffer();
}

unsigned int MultimediaPlayer::nextSegmentNrToConsume() {
  return m_buffer->nextSegmentNrToBeConsumed();
}

void MultimediaPlayer::SetLastDownloadBitRate(double bitrate) {
  this->m_lastBitrate = bitrate;
}

double MultimediaPlayer::GetLastDownloadBitRate() {
  return this->m_lastBitrate;
}

void MultimediaPlayer::traverseBuff() { this->m_buffer->traverseBuff(); }
