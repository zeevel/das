...
ISegmentURL *
SVCNoAdaptationLogic::GetNextSegment (unsigned int *requested_segment_number,
  ...
  if (next_segment_number < getTotalSegments ())
    {
      ...
      *hasDownloadedAllSegments = false;
      return m_orderdByDepIdReps[chosen_layer]->GetSegmentList ()->GetSegmentURLs ().at (
          next_segment_number);
    }
...
bool
SVCNoAdaptationLogic::hasMinBufferLevel (const dash::mpd::IRepresentation *rep)
{
...
void
SVCNoAdaptationLogic::SetAvailableRepresentations (
...
unsigned int
SVCNoAdaptationLogic::getNextNeededSegmentNumber (int layer)
{
  // check buffer
...
void
SVCNoAdaptationLogic::orderRepresentationsByDepIds ()
{
