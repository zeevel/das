...
#include "../model/multimediabuffer.h"
...
ISegmentURL *SVCNoAdaptationLogic::GetNextSegment(
  ...
  // GetBufferLevel(layerid): 获取缓存中具有对应质量的段数或时间
  // GetBufferLevel(0)：判断缓存中具备base layer的段数或时间
  // 即判断缓存是否为空
  double buffer_level_lowest_layer =
      m_multimediaPlayer->GetBufferLevel(m_orderdByDepIdReps[0]->GetId());
  // fprintf(stderr, "buffer_level_lowest_layer = %f\n",
  // buffer_level_lowest_layer);

  if (buffer_level_lowest_layer == 0.0) {  // 如果缓存为空 下载基本层
    next_segment_number = getNextNeededSegmentNumber(0);  // 下载哪个段的基本层
    chosen_layer = 0;  // 下base layer 基本层为空
    segment_number_set = true;
  } else  // find layer with lowest buffered seconds 如果具备base layer的段数 >
          // 具备higher layer的段数
  {
    unsigned int l = 1;
    while (l <= max_layer) {
      if (buffer_level_lowest_layer >
          m_multimediaPlayer->GetBufferLevel(m_orderdByDepIdReps[l]->GetId())) {
  ...
  if (next_segment_number > 1) {
    m_multimediaPlayer->traverseBuff();
  }
  ...
  if (next_segment_number < getTotalSegments()) {
    ...
    *hasDownloadedAllSegments = false;
    return m_orderdByDepIdReps[chosen_layer]
        ->GetSegmentList()
        ->GetSegmentURLs()
        .at(next_segment_number);
  }
...
bool SVCNoAdaptationLogic::hasMinBufferLevel(
    const dash::mpd::IRepresentation *rep) {
...
void SVCNoAdaptationLogic::SetAvailableRepresentations(
...
unsigned int SVCNoAdaptationLogic::getNextNeededSegmentNumber(int layer) {
  // std::cout << m_multimediaPlayer->m_buffer->buff << std::endl;
  // check buffer
  if (m_multimediaPlayer->GetBufferLevel(m_orderdByDepIdReps[layer]->GetId()) ==
      0) {
    ...
    return m_multimediaPlayer->getHighestBufferedSegmentNr(
               m_orderdByDepIdReps[layer]->GetId()) +
           1;
  }
}

void SVCNoAdaptationLogic::orderRepresentationsByDepIds() {
  std::map<std::string /*RepId*/, IRepresentation *> reps =
      *m_availableRepresentations;
  std::map<std::string /*RepId*/, IRepresentation *> selectedReps;

  // fprintf(stderr,
  // "m_availableRepresentations.size()=%d\n",m_availableRepresentations->size
  ...
  while (reps.size() > 0) {
    ...
    std::map<std::string /*RepId*/, IRepresentation *>::iterator lowest =
        reps.begin();
    for (std::map<std::string /*RepId*/, IRepresentation *>::iterator it =
             reps.begin();
         it != reps.end(); ++it) {
      if (lowest->second->GetDependencyId().size() <
          lowest->second->GetDependencyId().size())
        lowest = it;
    }
  ...
  // for(std::map<int /*level*/, IRepresentation*>::iterator it =
  // m_orderdByDepIdReps.begin (); it!=m_orderdByDepIdReps.end (); it++)
