...
ISegmentURL *
AbstractSVCBufferBasedAdaptationLogic::GetNextSegment (
    unsigned int *requested_segment_number, const dash::mpd::IRepresentation **usedRepresentation,
    bool *hasDownloadedAllSegments)
{
  ...
  while (m_multimediaPlayer->GetBufferLevel (m_orderdByDepIdReps[i_curr]->GetId ()) == 0.0 &&
         i_curr > 0)
    {
      i_curr--;
    }
  ...
  while (i <= i_curr)
    {
      if (m_multimediaPlayer->GetBufferLevel (m_orderdByDepIdReps[i]->GetId ()) <
          desired_buffer_size (i, i_curr))
        {
          ...
          if (next_segment_number < getTotalSegments ())
            {
              ...
              *hasDownloadedAllSegments = false;
              return m_orderdByDepIdReps[i]->GetSegmentList ()->GetSegmentURLs ().at (
                  next_segment_number);
            }
          else
  ...
  while (i <= i_curr)
    {
      if (m_multimediaPlayer->GetBufferLevel (m_orderdByDepIdReps[i]->GetId ()) <
          desired_buffer_size (i, i_curr + 2))
        {
          // i is the next, need segment number though
          next_segment_number = getNextNeededSegmentNumber (i);
...
void
AbstractSVCBufferBasedAdaptationLogic::SetAvailableRepresentations (
    std::map<std::string, IRepresentation *> *availableRepresentations)
{
  ...
  segment_duration =
      (double) m_orderdByDepIdReps.begin ()->second->GetSegmentList ()->GetDuration ();
  segment_duration /=
      (double) m_orderdByDepIdReps.begin ()->second->GetSegmentList ()->GetTimescale ();
}

//this functions classifies reps into layers depending on the DepIds.
void
AbstractSVCBufferBasedAdaptationLogic::orderRepresentationsByDepIds ()
{
  ...
  while (reps.size () > 0)
    {
      ...
      for (std::map<std::string /*RepId*/, IRepresentation *>::iterator it = reps.begin ();
           it != reps.end (); ++it)
        {
...
unsigned int
AbstractSVCBufferBasedAdaptationLogic::desired_buffer_size (int i, int i_curr)
{
  return gamma + (int) std::ceil ((i_curr - i) * alpha);
}

unsigned int
AbstractSVCBufferBasedAdaptationLogic::getNextNeededSegmentNumber (int layer)
{
  // check buffer
  if (m_multimediaPlayer->GetBufferLevel (m_orderdByDepIdReps[layer]->GetId ()) == 0)
    {
      ...
      if (layer == 0)
      ...
      else
        {
          ...
          return this->m_multimediaPlayer->nextSegmentNrToConsume () +
                 floor ((double) gamma / segment_duration);
        }
    }
  else
    {
...
bool
AbstractSVCBufferBasedAdaptationLogic::hasMinBufferLevel (const dash::mpd::IRepresentation *rep)
{
  ...
  int layer = -1;
  for (std::map<int /*level*/, IRepresentation *>::iterator it = m_orderdByDepIdReps.begin ();
       it != m_orderdByDepIdReps.end (); ++it)
    {
  ...
  if (m_multimediaPlayer->GetBufferLevel (m_orderdByDepIdReps[layer - 1]->GetId ()) <
      desired_buffer_size (layer - 1, layer))
    return false;
