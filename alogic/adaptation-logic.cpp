...
void
AdaptationLogic::SetAvailableRepresentations (
...
ISegmentURL *
AdaptationLogic::GetNextSegment (unsigned int *requested_segment_number,
...
unsigned int
AdaptationLogic::getTotalSegments ()
{
  return m_availableRepresentations->begin ()->second->GetSegmentList ()->GetSegmentURLs ().size ();
}

bool
AdaptationLogic::hasMinBufferLevel (const dash::mpd::IRepresentation *rep)
{
  return true;
}
