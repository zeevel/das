...
ISegmentURL *
RateAndBufferBasedAdaptationLogic::GetNextSegment (
    ...
    bool *hasDownloadedAllSegments)
{
  ...
  else if (this->m_multimediaPlayer->GetBufferLevel () >= 4 &&
           this->m_multimediaPlayer->GetBufferLevel () < 8)
    {
      factor = 0.50;
    }
  else if (this->m_multimediaPlayer->GetBufferLevel () >= 8 &&
           this->m_multimediaPlayer->GetBufferLevel () < 16)
    {
      factor = 0.75;
    }
  else
