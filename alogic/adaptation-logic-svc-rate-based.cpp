...
ISegmentURL *
SVCRateBasedAdaptationLogic::GetNextSegment (unsigned int *requested_segment_number,
                                             ...
                                             bool *hasDownloadedAllSegments)
{
  ...
  if (!hasMinBufferLevel () || //check if we have the minbuffer level
      (repsForCurSegment.empty () &&
       m_multimediaPlayer->GetBufferLevel () <
           bufMinLevel +
               GrowingBuffer)) // check if stack is empty if we should download a rep based on EMA bitrate
    {
  ...
  else if (repsForCurSegment.empty ()) // we need new stack for downloading
    {
      ...
      if (m_multimediaPlayer->GetBufferPercentage () >
          0.5) // we dont need to "lower" bitrate when buffer is nearly empty as we have a MinBufferCheck before
        {
      ...
      for (std::map<int /*level/layer*/, IRepresentation *>::iterator it =
               m_orderdByDepIdReps.begin ();
...
void
SVCRateBasedAdaptationLogic::updateEMA ()
{
  ema_download_bitrate = m_multimediaPlayer->GetLastDownloadBitRate () * RateBasedEMA_W +
                         (1 - RateBasedEMA_W) * ema_download_bitrate;
}

bool
SVCRateBasedAdaptationLogic::hasMinBufferLevel (const dash::mpd::IRepresentation *)
{
  return hasMinBufferLevel ();
}

bool
SVCRateBasedAdaptationLogic::hasMinBufferLevel ()
{
...
void
SVCRateBasedAdaptationLogic::SetAvailableRepresentations (
...
void
SVCRateBasedAdaptationLogic::orderRepresentationsByDepIds ()
{
