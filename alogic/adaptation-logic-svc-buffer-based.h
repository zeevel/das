...
class AbstractSVCBufferBasedAdaptationLogic : public AdaptationLogic {
 public:
  AbstractSVCBufferBasedAdaptationLogic(MultimediaPlayer *mPlayer,
                                        double buffer_alpha,
                                        double buffer_min_size)
      : AdaptationLogic(mPlayer) {
    alpha = buffer_alpha;
    gamma = buffer_min_size;
  }

  virtual void SetAvailableRepresentations(
 ...
 protected:
  ...
  enum AdaptationPhase { Steady = 0, Growing = 1, Upswitching = 2 };

  AdaptationPhase lastPhase;
  AdaptationPhase allowedPhase;
};

class SVCBufferBasedAdaptationLogicAggressive
    : AbstractSVCBufferBasedAdaptationLogic {
 public:
  ...
  static AdaptationLogic *Create(MultimediaPlayer *mPlayer) {
    return new AbstractSVCBufferBasedAdaptationLogic(
        mPlayer, BUFFER_ALPHA_AGGRESSIVE, BUFFER_MIN_SIZE_AGGRESSIVE);
  }
...
class SVCBufferBasedAdaptationLogicConservative
    : AbstractSVCBufferBasedAdaptationLogic {
 public:
  virtual std::string GetName() const {
  ...
  static AdaptationLogic *Create(MultimediaPlayer *mPlayer) {
    return new AbstractSVCBufferBasedAdaptationLogic(
        mPlayer, BUFFER_ALPHA_CONSERVATIVE, BUFFER_MIN_SIZE_CONSERVATIVE);
  }
 ...
 protected:
  ...
  SVCBufferBasedAdaptationLogicConservative() {
    ENSURE_ADAPTATION_LOGIC_REGISTERED(
        SVCBufferBasedAdaptationLogicConservative);
  }
};

class SVCBufferBasedAdaptationLogicNormal
    : AbstractSVCBufferBasedAdaptationLogic {
 public:
  virtual std::string GetName() const {
  ...
  static AdaptationLogic *Create(MultimediaPlayer *mPlayer) {
    return new AbstractSVCBufferBasedAdaptationLogic(
        mPlayer, BUFFER_ALPHA_NORMAL, BUFFER_MIN_SIZE_NORMAL);
  }
